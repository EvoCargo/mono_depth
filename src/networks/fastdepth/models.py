...
def convt(in_channels, out_channels, kernel_size):
    ...
    assert (
        -2 - 2 * padding + kernel_size + output_padding == 0
    ), "deconv parameters incorrect"
    return nn.Sequential(
        nn.ConvTranspose2d(
            in_channels,
            out_channels,
...
class ResNet(nn.Module):
    def __init__(self, layers, decoder, output_size, in_channels=3, pretrained=True):
        ...
        if layers not in [18, 34, 50, 101, 152]:
            raise RuntimeError(
                'Only 18, 34, 50, 101, and 152 layer model are defined for ResNet. Got {}'.format(
                    layers
                )
            )
        ...
        pretrained_model = torchvision.models.__dict__['resnet{}'.format(layers)](
    ...
    def __init__(self, decoder, output_size, in_channels=3, pretrained=True):
        ...
        if in_channels == 3:
            ...
            self.mobilenet = nn.Sequential(
                conv_bn(in_channels, 32, 2), *(mobilenet.model[i] for i in range(1, 14))
            )
...
class MobileNetSkipAdd(nn.Module):
    def __init__(self, output_size, pretrained=True):
        ...
        self.decode_conv1 = nn.Sequential(
            depthwise(1024, kernel_size), pointwise(1024, 512)
        )
        self.decode_conv2 = nn.Sequential(
            depthwise(512, kernel_size), pointwise(512, 256)
        )
        self.decode_conv3 = nn.Sequential(
            depthwise(256, kernel_size), pointwise(256, 128)
        )
...
class MobileNetSkipConcat(nn.Module):
    def __init__(self, output_size, pretrained=True):
        ...
        self.decode_conv3 = nn.Sequential(
            depthwise(512, kernel_size), pointwise(512, 128)
        )
