...
def run_model(
    model_dir, input_fp, output_fp, warmup_trials, run_trials, cuda, try_randin
):
    ...
    loaded_params = bytearray(
        open(os.path.join(model_dir, "deploy_param.params"), "rb").read()
    )
    ...
    print(
        "=> [TVM on TX2] benchmarking: {} warmup, {} run trials".format(
            warmup_trials, run_trials
        )
    )
    ...
    print(
        "=> [TVM on TX2] profiled runtime (in ms): {:.5f}".format(1000 * profiled_runtime)
    )
    ...
    if try_randin:
        ...
        print(
            "=> [TVM on TX2] with randomized input on every run, profiled runtime (in ms): {:.5f}".format(
                1000 * randomized_input_runtime
            )
        )
...
def main():
    ...
    parser.add_argument(
        '--cuda', type=bool, default=False, help='run with CUDA (default: False)'
    )
