...
def read_tum_trajectory_file(file_path):
    ...
    if not hasattr(file_path, 'read'):  # if not file handle
        logger.debug("Loaded {} stamps and poses from: {}".format(len(stamps), file_path))
...
def get_supported_topics(bag_handle):
    ...
    return sorted(
        [t for t in topic_info[1].keys() if topic_info[1][t][0] in SUPPORTED_ROS_MSGS]
    )
...
def read_bag_trajectory(bag_handle, topic):
    ...
    if msg_type not in SUPPORTED_ROS_MSGS:
        raise FileInterfaceException("unsupported message type: {}".format(msg_type))
    ...
    stamps, xyz, quat = [], [], []
    for _, msg, _ in bag_handle.read_messages(topic):
        ...
        quat.append(quat_t)
    logger.debug(
        "Loaded {} {} messages of topic: {}".format(len(stamps), msg_type, topic)
    )
