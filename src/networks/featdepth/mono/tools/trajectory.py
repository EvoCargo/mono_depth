...
import mono.tools.geometry as geometry
import mono.tools.lie_algebra as lie
import mono.tools.transformations as tr
import numpy as np
...
class PosePath3D(object):
    ...
    def __init__(
        self, positions_xyz=None, orientations_quat_wxyz=None, poses_se3=None, meta=None
    ):
        """
    ...
    def __eq__(self, other):
        ...
        equal = True
        equal &= all(
            [np.allclose(p1, p2) for p1, p2 in zip(self.poses_se3, other.poses_se3)]
        )
    ...
    def get_orientations_euler(self, axes="sxyz"):
        if hasattr(self, "_poses_se3"):
            return np.array([tr.euler_from_matrix(p, axes=axes) for p in self._poses_se3])
    ...
    def transform(self, t, right_mul=False, propagate=False):
        ...
        self._positions_xyz, self._orientations_quat_wxyz = se3_poses_to_xyz_quat_wxyz(
            self.poses_se3
        )
...
def calc_speed(xyz_1, xyz_2, t_1, t_2):
    ...
    """
    if (t_2 - t_1) <= 0:
        print("bad timestamps: " + str(t_1) + " & " + str(t_2))
