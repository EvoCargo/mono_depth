...
class mono_fm_joint(nn.Module):
    ...
    def compute_losses(self, inputs, outputs, features):
        ...
        for i in range(5):
            ...
            loss_dict[('feature_regularization_loss', i)] = (
                regularization_loss / (2 ** i) / 5
            )
        ...
        for scale in self.opt.scales:
            ...
            target_resize = F.interpolate(
                target, [h, w], mode="bilinear", align_corners=False
            )
            img_reconstruct_loss = self.compute_reprojection_loss(res_img, target_resize)
            loss_dict[
                ('img_reconstruct_loss', scale)
            ] = img_reconstruct_loss.mean() / len(self.opt.scales)
    ...
    def get_feature_regularization_loss(self, feature, img):
        ...
        smooth1 = torch.mean(
            feature_dx.abs() * torch.exp(-img_dx.abs().mean(1, True))
        ) + torch.mean(feature_dy.abs() * torch.exp(-img_dy.abs().mean(1, True)))

        smooth2 = (
            torch.mean(feature_dxx.abs() * torch.exp(-img_dxx.abs().mean(1, True)))
