...
def odo(opt):
    if opt.kitti:
        filenames = readlines(
            "../mono/datasets/splits/odom/test_files_{:02d}.txt".format(opt.sequence_id)
        )
    ...
    dataloader = DataLoader(
        dataset, 1, shuffle=False, num_workers=4, pin_memory=True, drop_last=False
    )
    ...
    for name, _ in pose_encoder.state_dict().items():
        pose_encoder.state_dict()[name].copy_(
            checkpoint['state_dict']['PoseEncoder.' + name]
        )
    for name, _ in pose_decoder.state_dict().items():
    ...
    with torch.no_grad():
        for _, inputs in enumerate(dataloader):
            ...
            backward_transform = (
                g.squeeze().cpu().numpy()
            )  # the transformation from frame +1 to frame 0
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        '--model_path',
        default='/media/sconly/24eda5d5-e79b-423b-8dcc-8339a15f3219/weight/fm_depth_odom.pth',
        help='model save path',
    )
    parser.add_argument(
        '--data_path',
        default='/media/sconly/24eda5d5-e79b-423b-8dcc-8339a15f3219/data/kitti/Odometry',
        help='kitti odometry dataset path',
    )
    parser.add_argument(
        '--gt_dir', default='../mono/datasets/gt_pose', help='kitti odometry gt path'
    )
    parser.add_argument(
        '--result_dir', default='/media/sconly/24eda5d5-e79b-423b-8dcc-8339a15f3219/odom/'
    )
    parser.add_argument('--height', default=192)
    parser.add_argument('--width', default=640)
    parser.add_argument(
        '--kitti', default=True, help='whether test on the kitti odometry dataset'
    )
    parser.add_argument(
        '--sequence_id', default=9, help='which kitti odometry sequence for testing'
    )
    opts = parser.parse_args()
    odo(opts)
    print(
        "you can also run 'evo_traj kitti -s *.txt *.txt --ref=*.txt -p --plot_mode=xz' in terminal for visualization"
    )
