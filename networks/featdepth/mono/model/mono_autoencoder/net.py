...
class autoencoder(nn.Module):
        ...
        for scale in self.opt.scales:
            ...
            if self.count % interval == 0:
                img_path = os.path.join(
                    '/media/data/datasets/penitto/networks/featdepth/test_images',
                    'auto_{:0>4d}_{}.png'.format(self.count // interval, scale),
                )
                plt.imsave(
                    img_path, pred[0].transpose(0, 1).transpose(1, 2).data.cpu().numpy()
                )
                img_path = os.path.join(
                    '/media/data/datasets/penitto/networks/featdepth/test_images',
                    'img_{:0>4d}_{}.png'.format(self.count // interval, scale),
                )
                plt.imsave(
                    img_path, target[0].transpose(0, 1).transpose(1, 2).data.cpu().numpy()
                )
    ...
    def get_smooth_loss(self, disp, img):
        ...
        smooth1 = torch.mean(
            disp_dx.abs() * torch.exp(-img_dx.abs().mean(1, True))
        ) + torch.mean(disp_dy.abs() * torch.exp(-img_dy.abs().mean(1, True)))

        smooth2 = (
            torch.mean(disp_dxx.abs() * torch.exp(-img_dxx.abs().mean(1, True)))
