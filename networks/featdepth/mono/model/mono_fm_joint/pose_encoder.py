...
def resnet_multiimage_input(num_layers, num_input_images=2, pretrained_path=None):
    assert num_layers in [
        ...
        50,
        101,
    ], "Can only run with 18, 34, 50, 101 layers resnet"
    blocks = {
        18: [2, 2, 2, 2],
        34: [3, 4, 6, 3],
        50: [3, 4, 6, 3],
    ...
    if num_layers < 40:
        model = ResNetMultiImageInput(
            BasicBlock, blocks, num_input_images=num_input_images
        )
    elif num_layers > 40:
        model = ResNetMultiImageInput(
            Bottleneck, blocks, num_input_images=num_input_images
        )
    ...
    def __init__(self, num_layers, pretrained_path=None, num_input_images=2):
        ...
        if num_input_images > 1:
            self.encoder = resnet_multiimage_input(
                num_layers, num_input_images, pretrained_path
            )
        else:
