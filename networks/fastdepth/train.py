...
def create_data_loaders(args):
    ...
    # load kitti datasets
        ...
        raise RuntimeError(
            'Dataset not found.' + 'The dataset must be either of nyudepthv2 or kitti.'
        )
...
def main():
    ...
    if args.evaluate:
        ...
        assert os.path.isfile(args.evaluate), "=> no model found at '{}'".format(
            args.evaluate
        )
    ...
    # resume from a particular check point
        ...
        assert os.path.isfile(chkpt_path), "=> no checkpoint found at '{}'".format(
            chkpt_path
        )
    ...
    for epoch in range(start, args.epochs):
        ...
        is_best = (
            result.rmse < best_result.rmse
        )  # compare result of the current epoch and best result
        if is_best:
            best_result = result
