...
def convert_image(i, scene, depth_raw, image):
    ...
    if idx in train_images:
        ...
        assert idx in test_images, (
            "index %d neither found in training set nor in test set" % idx
        )
        train_test = "test"
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 4:
        print(
            "usage: %s <h5_file> <train_test_split> <out_folder>" % sys.argv[0],
            file=sys.stderr,
        )
        sys.exit(0)
    ...
    scenes = [
        u''.join(chr(c) for c in h5_file[obj_ref]) for obj_ref in h5_file['sceneTypes'][0]
    ]
