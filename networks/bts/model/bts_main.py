...
# Multi-gpu training
parser.add_argument(
    '--num_threads', type=int, help='number of threads to use for data loading', default=1
)
parser.add_argument(
...
parser.add_argument(
    ...
    type=str,
    help='output directory for eval summary,' 'if empty outputs to checkpoint folder',
    default='',
)
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if not args.multiprocessing_distributed or (
        ...
        writer = SummaryWriter(
            args.log_directory + '/' + args.model_name + '/summaries', flush_secs=30
        )
    ...
    print(
        "Initial variables' sum: {:.3f}, avg: {:.3f}".format(var_sum, var_sum / var_cnt)
    )
    ...
    while epoch < args.num_epochs:
        ...
        for step, sample_batched in enumerate(dataloader.data):
            ...
            if (
                not args.do_online_eval
                and global_step
                and global_step % args.save_freq == 0
            ):
                if not args.multiprocessing_distributed or (
                    args.multiprocessing_distributed and args.rank % ngpus_per_node == 0
                ):
                ...
                if eval_measures is not None:
                    for i in range(9):
                        ...
                        if is_best:
                            ...
                            model_path = (
                                args.log_directory + '/' + args.model_name + old_best_name
                            )
